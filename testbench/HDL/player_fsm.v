...
module player_fsm (
    ...
    output reg         looking_right, // Bu sinyalin değeri artık sabit olacak.
    output reg         is_in_recovery,
    output reg         is_holding_backward,
    output reg         is_actively_blocking
);

    // Parametreler ve Durum Tanımları (Değişiklik yok)
    ...
    localparam P_STATE_OUT_ATTACKING = 3'b100, P_STATE_OUT_CHARGING = 3'b101, P_STATE_OUT_HITSTUN = 3'b110, P_STATE_OUT_BLOCKSTUN = 3'b111;

    // Dahili Kaydediciler (Değişiklik yok)
    ...
    always @(*) begin
        ...
        case (current_state_reg)
            ...
            S_N_ATK_STARTUP, S_M_ATK_STARTUP: begin
                player_state = P_STATE_OUT_ATTACKING;
                if (hit_by_opponent) next_state_reg = S_HITSTUN;
                else if (attack_frame_counter_reg == (current_state_reg == S_N_ATK_STARTUP ? N_ATK_STARTUP_FRAMES : M_ATK_STARTUP_FRAMES) - 1) 
                    next_state_reg = (current_state_reg == S_N_ATK_STARTUP ? S_N_ATK_ACTIVE : S_M_ATK_ACTIVE);
            end
            S_N_ATK_ACTIVE, S_M_ATK_ACTIVE: begin
                player_state = P_STATE_OUT_ATTACKING;
                hitbox_active = 1'b1;
                if (hit_by_opponent) next_state_reg = S_HITSTUN;
                else if (attack_frame_counter_reg == (current_state_reg == S_N_ATK_ACTIVE ? N_ATK_ACTIVE_FRAMES : M_ATK_ACTIVE_FRAMES) - 1) 
                    next_state_reg = (current_state_reg == S_N_ATK_ACTIVE ? S_N_ATK_RECOVERY : S_M_ATK_RECOVERY);
            end
            S_N_ATK_RECOVERY, S_M_ATK_RECOVERY: begin
                player_state = P_STATE_OUT_ATTACKING; is_in_recovery = 1'b1;
                if (hit_by_opponent) next_state_reg = S_HITSTUN;
                else if (attack_frame_counter_reg == (current_state_reg == S_N_ATK_RECOVERY ? N_ATK_RECOVERY_FRAMES : M_ATK_RECOVERY_FRAMES) - 1)
                    next_state_reg = S_IDLE;
            end
            S_HITSTUN: begin
    ...
    always @(posedge clk_game_logic or posedge reset) begin
        if (reset) begin
            ...
            // ==================== YENİ DÜZENLEME ====================
            // `looking_right` değerini güncelleyen aşağıdaki tüm `if-else` bloğu KALDIRILDI.
            // Bu sayede karakterlerin yönü oyun boyunca sabit kalır.
            /*
            if (next_state_reg == S_PRE_ATTACK_CHARGE && current_state_reg != S_PRE_ATTACK_CHARGE) begin
                ...
            end else if (current_state_reg == S_IDLE) begin
                ...
            end
            */
            // =========================================================
